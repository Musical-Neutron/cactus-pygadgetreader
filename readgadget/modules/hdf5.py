...
def hdf5_general(f, h, ptype):
    ...
    if ("PartType%d" % ptype) in f:
        if h.reading not in HDF5_NAMES:
            ...
            if h.debug:
                print("reading PartType%d/%s" % (ptype, HDF5_NAMES[h.reading]))
            arr = f["PartType%d/%s" % (ptype, HDF5_NAMES[h.reading])]

        else:
            if h.debug:
                print("could not locate PartType%d/%s" % (ptype, HDF5_NAMES[h.reading]))
        ...
        if h.units and h.reading == "u":
            ...
            if HDF5_NAMES["ne"] not in f["PartType0"]:
                print(
                    "WARNING! ElectronAbundance not found!  Temp estimate approximate"
                )
        ...
        if h.debug:
            print("coult not find PartType%d" % ptype)
        arr = np.zeros(0, dtype=np.float32)

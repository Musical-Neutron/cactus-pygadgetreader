...
def skipblocks(f, h, val):
    ...
    for key, items in h.BLOCKORDER.items():
        ...
        if val == "metalarray" and key == "metallicity":
            return
...
def gadget_readmetals(f, h, ptype, single=1):
    ...
    metals = np.fromfile(
        f, dtype=h.dataType, count=h.flag_metals * h.npartThisFile[ptype]
    )
    if ptype == 0:
...
def gadget_read(f, h, p, d):
    ...
    if h.reading == "pos" or h.reading == "vel":
        ...
        if p != 0:
            print(
                "WARNING!! you requested ParticleType%d for %s, returning GAS instead"
                % (p, h.reading)
            )

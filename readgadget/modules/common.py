...
def initUnits(h):
    ...
    if h.units and h.fileType != "tipsy":
        if h.reading == "rho":
            if h.boxsize > 0.0 and h.OmegaLambda > 0:
                convert = (
                    (1.0 + h.redshift) ** 3 * h.UnitMass_in_g / h.UnitLength_in_cm**3
                )
            else:
                convert = h.UnitMass_in_g / h.UnitLength_in_cm**3
        elif h.reading == "vel":
            if h.boxsize > 0.0 and h.Ol > 0:
            ...
            h.UnitEnergy_in_cgs = (
                h.UnitMass_in_g * h.UnitLength_in_cm**2 / h.UnitTime_in_s**2
            )
        elif h.reading == "mass":
...
def getTfactor(Ne, h):
    ...
    MeanWeight = 4.0 / (3.0 * H_MASSFRAC + 1.0 + 4.0 * H_MASSFRAC * Ne) * PROTONMASS
    conversion = (
        MeanWeight / BOLTZMANN * (GAMMA - 1.0) * h.UnitEnergy_in_cgs / h.UnitMass_in_g
    )
    return conversion
